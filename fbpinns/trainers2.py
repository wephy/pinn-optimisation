...
class PINNTrainerBFGS(_Trainer):
    "PINN model trainer class"
    ...
    def train(self):
        ...
        # --- 1. Standard PINN Initialization ---
        key = jax.random.PRNGKey(c.seed)
        np.random.seed(c.seed)
        all_params = {"static":{}, "trainable":{}}
        domain, problem, network = c.domain, c.problem, c.network

        # Initialize domain, problem, and network parameters (same as PINNTrainer)
        for tag, cl, kwargs in zip(["domain", "problem"], [domain, problem],
                                   [c.domain_init_kwargs, c.problem_init_kwargs]):
            ps_ = cl.init_params(**kwargs)
            if ps_[0]: all_params["static"][tag] = ps_[0]
        ...
        mu_, sd_ = c.decomposition_init_kwargs.get("unnorm", (0., 1.))
        unnorm_fn = lambda u: networks.unnorm(mu_, sd_, u)
        model_fns = (domain.norm_fn, network.network_fn, unnorm_fn, problem.constraining_fn)

        (optimiser, all_opt_states, optimiser_fn, loss_fn, key,
         constraints, x_batch_global, _, _, jmapss,
         x_batch_test, u_exact) = _common_train_initialisation(c, key, all_params, problem, domain)
        
        # --- 2. Compile Adam Step ---
        logger.info(f"Compiling Adam update step for the first {c.adam_steps} steps...")
        active_params = all_params["trainable"]
        static_params = all_params["static"]
        static_params_dynamic, static_params_static = partition(static_params)
        
        # AOT compile the Adam update function
        adam_update_step = PINN_update.lower(
            optimiser_fn, all_opt_states, active_params, static_params_dynamic, 
            static_params_static, constraints, model_fns, jmapss, loss_fn
        ).compile()
        logger.info("Adam compilation done.")
        ...
        # BFGS solver and state, initialized to None before the loop
        bfgs_solver = None
        bfgs_state = None
        
        # Set active_opt_states for the Adam phase
        active_opt_states = all_opt_states

        for i in range(c.n_steps):
            if i == 0:
                # Report initial model (before any steps)
                self._report(i, pstep, fstep, u_test_losses, start0, start1, report_time,
                             u_exact, x_batch_test, all_params, active_opt_states, model_fns, problem,
                             active_params, x_batch_global, lossval)

            # --- Training Step ---
            if i < c.adam_steps:
                # ADAM PHASE
                lossval, active_opt_states, active_params = adam_update_step(
                    active_opt_states, active_params, static_params_dynamic
                )
            else:
                # BFGS PHASE
                if bfgs_solver is None:
                    logger.info(f"[i: {i}/{c.n_steps}] Initializing BFGS solver...")
                    
                    # Define the loss function for BFGS. It takes trainable params
                    # and closes over all the static data.
                    def loss_for_bfgs(params):
                        return PINN_loss(params, static_params, constraints, model_fns, jmapss, loss_fn)
                    
                    bfgs_solver = jaxopt.BFGS(fun=loss_for_bfgs, jit=True, maxiter=500) # You can tune maxiter
                    bfgs_state = bfgs_solver.init_state(active_params)
                    logger.info("BFGS solver initialized.")

                # Run one step of the BFGS optimizer
                active_params, bfgs_state = bfgs_solver.update(
                    active_params,
                    state=bfgs_state
                )
                lossval = bfgs_state.error

            pstep += 1 # A single step is taken per iteration

            # --- Reporting ---
            # Update all_params with the latest trainable params for reporting
            all_params["trainable"] = active_params
            u_test_losses, start1, report_time = \
                self._report(i + 1, pstep, fstep, u_test_losses, start0, start1, report_time,
                             u_exact, x_batch_test, all_params, active_opt_states, model_fns, problem,
    ...
    def _report(self, i, pstep, fstep, u_test_losses, start0, start1, report_time,
                u_exact, x_batch_test, all_params, all_opt_states, model_fns, problem,
                active_params, x_batch, lossval): # <--- REMOVED active_opt_states
        "Report results"
...
class NonlinearCGTrainer(_Trainer):
    "A self-contained PINN trainer using the JAXopt Nonlinear Conjugate Gradient solver."

    def train(self):
        "Train model"
        c, writer = self.c, self.writer

        # --- Stages 1, 2, 3 (Initialization) ---
        key = random.PRNGKey(c.seed)
        np.random.seed(c.seed)
        all_params = {"static":{}, "trainable":{}}
        ...
        logger.info("Sampling training constraints...")
        key, subkey = random.split(key)
        constraints_with_ujs = problem.sample_constraints(all_params=all_params, domain=domain, key=subkey, sampler=c.sampler, batch_shapes=c.ns)
        
        required_ujss = [constraint_[-1] for constraint_ in constraints_with_ujs]
        constraints = [constraint_[:-1] for constraint_ in constraints_with_ujs]
        x_batch_global = jnp.concatenate([constraint_[0] for constraint_ in constraints])
        
        logger.info("Generating jacobian maps for the PINN...")
        jmapss = tuple(get_jmaps(required_ujs) for required_ujs in required_ujss)

        logger.info("Generating test grid and exact solution...")
        x_batch_test = domain.sample_interior(all_params=all_params, key=None, sampler="grid", batch_shape=c.n_test)
        u_exact = problem.exact_solution(all_params=all_params, x_batch=x_batch_test, batch_shape=c.n_test)
        
        # --- 4. Solver Initialization ---
        
        def loss_for_solver(active_params, static_params, constraints):
            return PINN_loss(active_params, static_params, constraints, model_fns, jmapss, loss_fn)
        
        solver = jaxopt.NonlinearCG(fun=loss_for_solver, method='polak-ribiere', jit=True)

        active_params = all_params["trainable"]
        static_params = all_params["static"]
        
        logger.info("Initializing NonlinearCG solver state...")
        solver_state = solver.init_state(active_params, static_params, constraints)
        ...
        lossval = loss_for_solver(active_params, static_params, constraints)
        logger.info(f"Initial loss: {lossval:.6f}")

        logger.info(f"Starting training with NonlinearCG for {c.n_steps} steps.")

        for i in range(c.n_steps):
            if i == 0:
                self._report(i, pstep, fstep, u_test_losses, start0, start1, report_time,
                                u_exact, x_batch_test, all_params, None, model_fns, problem,
                                None, active_params, x_batch_global, lossval)

            # Take a training step
            active_params, solver_state = solver.update(active_params,
                                                         state=solver_state,
                                                         static_params=static_params,
                                                         constraints=constraints)
            
            lossval = solver_state.error
            pstep += 1
    ...
    def _test(self, x_batch_test, u_exact, u_test_losses, x_batch, i, pstep, fstep, start0, all_params, model_fns, problem):
        "Test step. Includes plotting loss vs. epochs."
        ...
        # Original figures are plotted at a lower frequency
        if i % (c.test_freq * 5) == 0:
            orig_fs = plot_trainer.plot("PINN", all_params["static"]["problem"]["dims"],
                                        x_batch_test, u_exact, u_test, u_raw_test, x_batch, all_params, i, n_test)
            if orig_fs is not None:
                fs.update(orig_fs)
    
        # New plot: Loss vs. Epochs, generated at every test step
        if len(u_test_losses) > 1:
            # Create a new figure for the loss plot
            loss_fig, ax = plt.subplots(figsize=(8, 6))
            
            # Extract data for plotting
            losses_arr = jnp.array(u_test_losses)
            iterations = losses_arr[:, 0]
            l1_losses = losses_arr[:, 4]
    
            # Plot the data
            ax.plot(iterations, l1_losses, marker='o', linestyle='-', markersize=4, label='L1 Test Loss')
            
            # Formatting
            ax.set_xlabel("Epoch")
            ax.set_ylabel("L1 Test Loss")
            ax.set_title("Test Loss vs. Epochs")
            ax.set_yscale('log') # Loss is often best viewed on a log scale
            ax.grid(True, which="both", ls="--")
            ax.legend()
            
            # Add the figure to our dictionary to be saved by the writer
            fs['loss_vs_epoch'] = loss_fig
            plt.show()  # Close the figure handle to prevent it from displaying directly
    
        # Save all generated figures to the writer
        if fs:
            self._save_figs(i, fs.items())
...
class FBPINNNonlinearCGTrainer(_Trainer):
    """
    FBPINN model trainer that uses a hybrid optimization strategy:
    a few initial steps of Adam, followed by the Nonlinear Conjugate Gradient
    method from jaxopt.
    """
    ...
    def _get_x_batch(self, i, active, all_params, x_batch_global, constraints_global, constraint_fs_global, constraint_offsets_global, decomposition):
        """
        Get the x_batch points from x_batch_global which are inside active models.
        (Copied from FBPINNTrainer as it's a required helper)
        """
        # cut active points out of x_batch_global
        ims = jnp.arange(all_params["static"]["decomposition"]["m"])[active==1]
        training_ips, _d = decomposition.inside_models(all_params, x_batch_global, ims)# (n, mc)
    ...
    def _get_update_inputs(self, i, active, all_params, all_opt_states, x_batch_global, constraints_global, constraint_fs_global, constraint_offsets_global, decomposition, problem):
        ...
        if all_opt_states:
            active_opt_states = tree_map_dicts(cut_active, all_opt_states)
        else:
            active_opt_states = None
        ...
        for f, ips in zip(constraint_fs.T, constraint_ips):
            f1, f2 = f, f[np_take]
            ius, f3, f4 = iu_[f2], f1[n_take], f2[p_take]
            
            inv = jnp.zeros(x_batch.shape[0], dtype=int).at[ips].set(jnp.arange(len(ips)))
            inv2 = jnp.zeros(np_take.shape[0], dtype=int).at[ius].set(jnp.arange(len(ius)))
            
            m_take_ic, n_take_ic = m_take[f3], inv[n_take[f3]]
            p_take_ic, np_take_ic = inv2[p_take[f4]], inv[np_take[f2]]
            
            takess.append((m_take_ic, n_take_ic, p_take_ic, np_take_ic, npou))

        logger.info(f"Updating active inputs done ({time.time()-start0:.2f} s)")
        return active, merge_active, active_opt_states, active_params, fixed_params, static_params, takess, constraints, x_batch, all_ims

    def train(self):
        """Train model using a hybrid Adam + NonlinearCG approach."""
        c, writer = self.c, self.writer
        ...
        pstep, fstep, u_test_losses = 0, 0, []
        start0, start1, report_time = time.time(), time.time(), 0.
        lossval, p, f = None, 0, 0
        merge_active, active_params, all_ims = None, None, None
        solver, solver_state, update = None, None, None

        for i, active_ in enumerate(scheduler):
            if active_ is not None:
                active = active_
                if i > 0 and merge_active and active_params is not None:
                    all_params["trainable"] = merge_active(active_params, all_params["trainable"])
                    if i <= c.adam_steps:
                        all_opt_states = tree_map_dicts(merge_active, active_opt_states, all_opt_states)

                active, merge_active, active_opt_states, active_params, fixed_params, static_params, takess, constraints, x_batch, all_ims = \
                    self._get_update_inputs(i, active, all_params, all_opt_states, x_batch_global, constraints_global, constraint_fs_global, constraint_offsets_global, decomposition, problem)
                
                static_params_dynamic, static_params_static = partition(static_params)
                
                # Always recompile Adam step for the new active set
                startc = time.time()
                logger.info(f"[i: {i}/{self.c.n_steps}] Compiling Adam update step..")
                update = FBPINN_update.lower(optimiser_fn, active_opt_states,
                                             active_params, fixed_params, static_params_dynamic, static_params_static,
                                             takess, constraints, model_fns, jmapss, loss_fn).compile()
                logger.info(f"Compiling Adam done ({time.time()-startc:.2f} s)")
                p, f = total_size(active_params["network"]), flops_cost_analysis(update.cost_analysis())
                
                # Invalidate the solver so it gets re-initialized for the CG phase with the new data
                solver = None

            if i == 0:
                self._report(i, pstep, fstep, u_test_losses, start0, start1, report_time,
                             u_exact, x_batch_test, test_inputs, all_params, all_opt_states, model_fns, problem, decomposition,
                             active, merge_active, active_opt_states, active_params, x_batch, all_ims, lossval)

            # --- Training Step ---
            if i < c.adam_steps:
                lossval, active_opt_states, active_params = update(active_opt_states, active_params, fixed_params, 
                                                                   static_params_dynamic, takess, constraints)
            else:
                # Initialize or re-initialize the solver if it's the first CG step or if the active set has changed.
                if solver is None:
                    logger.info(f"[i: {i}/{self.c.n_steps}] Initializing/Re-initializing NonlinearCG solver.")
                    
                    # Define a loss function that accepts dynamic arguments for the solver.
                    def loss_for_cg(params, fixed_params, static_params, takess, constraints):
                        # The model and loss functions are closed over as they are static for this step.
                        return FBPINN_loss(params, fixed_params, static_params, takess, constraints, model_fns, jmapss, loss_fn)
                    
                    solver = jaxopt.NonlinearCG(fun=loss_for_cg, method='polak-ribiere', jit=True)
                    
                    # Initialize the solver state, passing dynamic data as positional arguments.
                    solver_state = solver.init_state(active_params, fixed_params, static_params, takess, constraints)

                # Pass dynamic data to the update function as positional arguments after `state`.
                active_params, solver_state = solver.update(active_params, 
                                                            solver_state, 
                                                            fixed_params, 
                                                            static_params, 
        ...
        writer.close()
        logger.info(f"Training complete")
        if merge_active and active_params is not None:
            all_params["trainable"] = merge_active(active_params, all_params["trainable"])
        return all_params
    ...
    def _report(self, i, pstep, fstep, u_test_losses, start0, start1, report_time,
        ...
        if summary_ or test_ or model_save_:
            if i > 0 and summary_ and lossval is not None:
                rate = c.summary_freq / (time.time() - start1 - report_time)
                self._print_summary(i, lossval.item(), rate, start0)
                start1, report_time = time.time(), 0.
            ...
            if test_ or model_save_:
                ...
                if test_:
                    u_test_losses = self._test(
                        x_batch_test, u_exact, u_test_losses, x_batch, test_inputs, i, pstep, fstep, start0, active, all_params, model_fns, problem, decomposition, all_ims)
                
                if model_save_:
                    # Note: all_opt_states might be stale if in CG phase, but we save it anyway.
                    self._save_model(i, (i, all_params, all_opt_states, active, jnp.array(u_test_losses)))
    ...
    def _test(self, x_batch_test, u_exact, u_test_losses, x_batch, test_inputs, i, pstep, fstep, start0, active, all_params, model_fns, problem, decomposition, all_ims):
        """Test step. Includes plotting loss vs. epochs."""
        c, writer = self.c, self.writer

        takes, _, cut_all = test_inputs
        ...
        if i % (c.test_freq * 5) == 0:
            if all_params["static"]["problem"]["dims"][1] == 1:
                m, ud, n = all_params["static"]["decomposition"]["m"], all_params["static"]["problem"]["dims"][0], x_batch_test.shape[0]
                us_test, ws_test, us_raw_test = jnp.full((3, m, n, ud), jnp.nan)
                us_test = us_test.at[all_ims[takes[0]], takes[1], :].set(us_test_)
                ws_test = ws_test.at[all_ims[takes[0]], takes[1], :].set(ws_test_)
                us_raw_test = us_raw_test.at[all_ims[takes[0]], takes[1], :].set(us_raw_test_)
                us_test = us_test.at[all_ims[takes[0]], takes[1], :].divide(wp_test_[takes[2]]) / takes[4]
            ...
            orig_fs = plot_trainer.plot("FBPINN", all_params["static"]["problem"]["dims"],
                                       x_batch_test, u_exact, u_test, us_test, ws_test, us_raw_test, x_batch, all_params, i, active, decomposition, c.n_test)
            if orig_fs: fs.update(orig_fs)
        
        if len(u_test_losses) > 1:
            loss_fig, ax = plt.subplots(figsize=(8, 6))
            losses_arr = jnp.array(u_test_losses)
            ax.plot(losses_arr[:, 0], losses_arr[:, 4], marker='o', linestyle='-', markersize=4, label='L1 Test Loss')
            ax.set(xlabel="Epoch", ylabel="L1 Test Loss", title="Test Loss vs. Epochs", yscale='log')
            ax.grid(True, which="both", ls="--")
            ax.legend()
            fs['loss_vs_epoch'] = loss_fig
            plt.show()
...
class FBPINNBFGSTrainer(_Trainer):
    """
    FBPINN model trainer that uses a hybrid optimization strategy:
    a few initial steps of Adam, followed by the Nonlinear Conjugate Gradient
    method from jaxopt.
    ...
    def train(self):
        ...
        for i, active_ in enumerate(scheduler):
            ...
            if i < c.adam_steps:
                ...
                # Initialize or re-initialize the solver if it's the first CG step or if the active set has changed.
                if solver is None:
                    logger.info(f"[i: {i}/{self.c.n_steps}] Initializing BFGS solver.")
                    ...
                    solver = jaxopt.BFGS(fun=loss_for_cg, jit=True)
...
class BlockNewton:
    """
    A Newton optimizer that computes only the block of the Hessian
    corresponding to the 'active' parameters, making it suitable for
    domain decomposition problems.
    """
    def __init__(self, fun):
        # fun is the global loss function, e.g., FBPINN_loss
        self.fun = fun

    def update(self, active_params, fixed_params, static_params, takess, constraints, model_fns, jmapss, loss_fn):
        """
        Performs an optimization step on the active_params.
        """

        # 1. Define a loss function wrapper for the Hessian calculation.
        #    Crucially, active_params is the first argument (argnums=0).
        #    The other parameters are passed but treated as constants by jax.hessian.
        def loss_for_hessian_block(act_p, fix_p, sta_p, tks, cons):
            return self.fun(act_p, fix_p, sta_p, tks, cons, model_fns, jmapss, loss_fn)

        # 2. Compute the gradient with respect to active parameters only.
        val, grad_active = jax.value_and_grad(loss_for_hessian_block, argnums=0)(
            active_params, fixed_params, static_params, takess, constraints
        )

        # 3. Flatten the gradient and prepare for the linear solve.
        grad_active_flat, unravel_fn = jax.flatten_util.ravel_pytree(grad_active)

        # 4. Define the Hessian-vector product (HVP) for the active block.
        #    This is still the most memory-efficient way to solve the linear system
        #    even when computing a block. It computes (H_block @ v).
        def hvp_active_block(v_flat):
            v_unraveled = unravel_fn(v_flat)
            # The 'jvp of a vjp' trick computes the HVP efficiently.
            _, hvp_unraveled = jax.jvp(
                lambda p: jax.grad(loss_for_hessian_block, argnums=0)(p, fixed_params, static_params, takess, constraints),
                (active_params,),
                (v_unraveled,)
            )
            hvp_flat, _ = jax.flatten_util.ravel_pytree(hvp_unraveled)
            return hvp_flat

        # 5. Solve the linear system H_block * p = -g_active using Conjugate Gradients.
        #    This finds the Newton step `p` without ever forming the Hessian block.
        step_flat, _ = cg(
            A=hvp_active_block,
            b=-grad_active_flat
        )

        # 6. Unflatten the step and apply the update.
        step_unraveled = unravel_fn(step_flat)
        new_active_params = jax.tree_util.tree_map(lambda p, s: p + s, active_params, step_unraveled)

        return new_active_params, val


class NewtonCG:
    """A memory-efficient Newton optimizer using Conjugate Gradients."""

    def __init__(self, fun):
        self.fun = fun

    def update(self, params, *args):
        """Performs a single optimization step using Hessian-vector products."""

        # 1. Flatten the pytree of parameters into a single vector.
        flat_params, unravel_fn = jax.flatten_util.ravel_pytree(params)

        # 2. Define a wrapper for the loss function that works with the flat vector.
        def fun_flat(p_flat, *a):
            return self.fun(unravel_fn(p_flat), *a)

        # 3. Compute the gradient of the flat function.
        val, grad_flat = jax.value_and_grad(fun_flat)(flat_params, *args)


class FBPINN_Newton(_Trainer):
    """
    FBPINN model trainer that uses a hybrid optimization strategy:
    a few initial steps of Adam, followed by Newton's method with a trust region.
    """
    ...
    def train(self):
        """Train model using a hybrid Adam + Newton's method with a trust-region approach."""
        c, writer = self.c, self.writer
        ...
        lossval, p, f = None, 0, 0
        merge_active, active_params, all_ims = None, None, None
        solver, update = None, None
        trust_radius = 0.5  # Initial trust radius
        ...
        for i, active_ in enumerate(scheduler):
            if active_ is not None:
                ...
                # Invalidate the solver so it gets re-initialized for the Newton phase
                solver = None
            ...
            if i < c.adam_steps:
                ...
                # Initialize or re-initialize the solver if it's the first Newton step or if the active set has changed.
                if solver is None:
                    logger.info(f"[i: {i}/{self.c.n_steps}] Initializing BlockNewton solver.")
                    # Pass the global FBPINN_loss function to the optimizer
                    solver = BlockNewton(fun=FBPINN_loss)

                # Perform a BlockNewton step
                active_params, lossval = solver.update(
                    active_params,
                    fixed_params,
                    static_params,
                    takess,
