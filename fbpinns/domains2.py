...
class RectangularDomainND(Domain):
    ...
    def _rectangle_samplerND(key, sampler, xmin, xmax, batch_shape):
        ...
        if not sampler in ["grid", "uniform", "sobol", "halton", "burgers"]:
        ...
        if sampler == "grid":
            ...
            # elif sampler == "burgers":
            #     sobol_generator = scipy.stats.qmc.Sobol(d=xd, seed=42)
            #     original_points = sobol_generator.random(np.prod(batch_shape))
            #     concentration_param = 2
            #     beta_dist = scipy.stats.beta(concentration_param, concentration_param)
            #     x_original = original_points[:, 0]
            #     y_original = original_points[:, 1]
            #     x_concentrated = beta_dist.ppf(x_original)
            #     s = np.stack((x_concentrated, y_original), axis=1)
